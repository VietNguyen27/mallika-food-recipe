...
const RecipeDetail = () => {
  ...
  useEffect(() => {
    if (recipeId && recipe[recipeId] && !loading) {
      const { title, time, image, serve, difficulty } = recipe[recipeId];
  ...
  if (recipeId && !recipe[recipeId]) return null;
  ...
  return (
    <>
              ...
              reviews[recipeId][0] ? (
                <div className='flex gap-2'>
                  <div className='relative w-8 h-8 flex-shrink-0 mt-1 rounded-full overflow-hidden'>
                    <img
                      src={generateBase64Image(
                        reviews[recipeId][0].user.avatar
                      )}
                      className='absolute w-full h-full object-cover'
                      alt={reviews[recipeId][0].user.name}
                    />
                  </div>
                  <div className='w-full'>
                    <div className='flex justify-between items-center pb-1'>
                      <h3 className='text-sm font-semibold'>
                        {reviews[recipeId][0].user.name}
                      </h3>
                    </div>
                      ...
                      {[...Array(5).keys()].map((_, index) => {
                        return (
                          <Star12Filled
                            key={index}
                            className={
                              index + 1 <= reviews[recipeId][0].rating
                                ? 'text-orange'
                                : 'text-gray-400'
                            }
                          />
                    ...
                    <p className='text-sm line-clamp-2'>
                      {reviews[recipeId][0].comment}
                    </p>
                    <p className='text-xs text-gray-500'>
                      {getFullDateTime(reviews[recipeId][0].createdAt)}
                    </p>
                  </div>
                </div>
              ) : (
      ...
      <ReviewDrawer recipeId={recipeId as string} />
    </>
  );
};
